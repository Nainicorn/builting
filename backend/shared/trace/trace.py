...
def write_trace_end(trace_key, stage, run_id, attempt_n, started_at, finished_at,
    ...
    scalars (optional dict): Release 13.2 observability scalars.
      gatePassRate (float|None): % of validation gates that passed (0-100).
      contractStatus ('pass'|'fail'|None): input contract check outcome.
      provenanceCompleteness (float|None): % of elements with non-missing provenance.
    """
    _scalars = scalars or {}
    s3_client.put_object(
        Bucket=IFC_BUCKET,
